...
async def skip(cli, message: Message, _, chat_id):
    ...
    if "live_" in queued:
        ...
        try:
            ...
            image = None
        try:
            await Legend.skip_stream(chat_id, link, video=status, image=image)
        except Exception:
        ...
        try:
            ...
            image = None
        try:
            await Legend.skip_stream(chat_id, file_path, video=status, image=image)
        except Exception:
            return await mystic.edit_text(_["call_9"])
        ...
        if videoid == "telegram":
            ...
            try:
                ...
                image = None
        try:
            await Legend.skip_stream(chat_id, queued, video=status, image=image)
        except Exception:
            return await message.reply_text(_["call_9"])
        if videoid == "telegram":
            button = telegram_markup(_, chat_id)
